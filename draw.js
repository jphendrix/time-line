  ...
  filter: function(args) {
    ...
    for (var key in args) {
      for (var i = 0; i < this[key].length; i++) {
        ...
        if ($.inArray(this[key][i].name, addedItems) >= 0) {
          continue;
        }

        //Apply this filter to any title associateed with this entry
        var titles = this[key][i].title.split(',').map(function(x) {
        ...
        for (var j = 0; j < titles.length; j++) {
          if ($.inArray(titles[j], args[key]) >= 0) {
            ...
            if (start < filtered.minDate) {
              filtered.minDate = start;
            }
            if (end > filtered.maxDate) {
              filtered.maxDate = end;
            }
